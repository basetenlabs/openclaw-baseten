---
summary: "Use Baseten Model APIs for high-performance LLMs in OpenClaw"
read_when:
  - You want high-performance LLM inference in OpenClaw
...
## Why Baseten in OpenClaw

- **High-performance LLMs** with optimized serving infrastructure.
- **Wide model selection** including Kimi K2.5, GPT OSS 120B, DeepSeek V3.1, Qwen3 Coder, GLM-4.7, and more.
...
```bash
openclaw chat --model baseten/moonshotai/Kimi-K2.5 "Hello, are you working?"
```

## Model Selection

OpenClaw includes a curated catalog of popular Baseten Model API models. Pick based on your needs:

- **Default**: `moonshotai/Kimi-K2.5` (Kimi K2.5) - reasoning-capable, 256k context.
...
```bash
openclaw models set baseten/moonshotai/Kimi-K2.5
openclaw models set baseten/openai/gpt-oss-120b
```
...
|----------|-------------------|-----|
| **General chat** | `moonshotai/Kimi-K2.5` | Reasoning-capable, 256k context (default) |
| **Complex reasoning** | `openai/gpt-oss-120b` | Best for step-by-step reasoning |
| **Agentic tasks** | `moonshotai/Kimi-K2.5` | Designed for reasoning and agentic use |
...
| `moonshotai/Kimi-K2.5` | Kimi K2.5 | 256k | Reasoning (default) |
| `moonshotai/Kimi-K2-Thinking` | Kimi K2 Thinking | 262k | Reasoning |
| `moonshotai/Kimi-K2-Instruct-0905` | Kimi K2 Instruct 0905 | 128k | General |
...
```bash
openclaw chat --model baseten/moonshotai/Kimi-K2.5
```
...
openclaw chat --model baseten/openai/gpt-oss-120b

# Use coding model
openclaw chat --model baseten/Qwen/Qwen3-Coder-480B-A35B-Instruct

# Use reasoning model with long context
openclaw chat --model baseten/moonshotai/Kimi-K2-Thinking
```
...
{
  env: { BASETEN_API_KEY: "..." },
  agents: { defaults: { model: { primary: "baseten/moonshotai/Kimi-K2.5" } } },
  models: {
    mode: "merge",
    providers: {
      baseten: {
